...
class MyMainGui(QWidget,Ui_Form):
    ...
    def local_right_menu(self, pos):
        ...
        # mkdir
            ...
            try:
                dir_name = QInputDialog.getText(self, 'nueva carpeta', 'ingrese el nombre de la carpeta', QLineEdit.Normal)
                if not dir_name[1]:
                    return
                ...
                message = QMessageBox.information(self,'la carpeta ya existe','el nombre de la carpeta ya existe, modifique el nombre del archivo y créelo de nuevo')
        ...
        # rename
            ...
            rename = QInputDialog.getText(self, 'cambiar nombre', 'muestra el nombre del archivo', QLineEdit.Normal)
            if not rename[1]:
                return
            pathname = os.path.join(self.local_pwd, str(item.text(0)))
            # pathname = pathname.replace('\\', '/')
        ...
        # new file
            ...
            file_name = QInputDialog.getText(self, 'nuevo archivo', 'muestra el nombre del archivo', QLineEdit.Normal)
            if not file_name[1]:
                return
            try:
                ...
                message = QMessageBox.information(self,'el archivo ya existe','el nombre del archivo ya existe, créalo de nuevo')
    ...
    def remote_right_menu(self, pos):
        ...
        if action == refresh:
            ...
            try:
                dir_name = QInputDialog.getText(self, 'crear carpeta', 'ingrese el nombre de la carpeta', QLineEdit.Normal)
                if not dir_name[1]:
                    return
                ...
                message = QMessageBox.information(self,'sin autorización','no tienes permiso para esta operación')
            ...
            def _remote_newfile():
                try:
                    ...
                    file.close()
                    os.remove(tmp_file)
                    print('no tienes permiso para esta operación')
